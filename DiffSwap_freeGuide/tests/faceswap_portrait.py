...
from src.gaze_estimation.gaze_estimator import Gaze_estimator
from src.parsing import BiSeNet
from src.module import SpecificNorm, cosin_metric
...
@torch.no_grad()
def perform_swap(self, batch, ckpt, ddim_sampler = None, dict_condfn={}, ddim_steps=200, ddim_eta=0., **kwargs):
    ...
    for mask_key in ['mask']:
        ...
        with self.ema_scope("Plotting Inpaint"):
            ...
            samples, _ = ddim_sampler.sample(ddim_steps,N,shape,c, eta=ddim_eta, x0=z[:N], mask=mask, verbose=False,dict_condfn=dict_condfn, decode = self.decode_first_stage, **kwargs)
        ...
        plot_loss(ddim_sampler.list_GazeLoss, 'gaze_loss.png')
        # plot_loss(ddim_sampler.list_SegmentLoss, 'segment_loss.png')
...
def plot_loss(list_lossGaze, name):
    plt.plot(list_lossGaze)
    plt.savefig(name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-gaze', '--guide_gaze', action='store_true')
    parser.add_argument('-f',  '--guide_fq', action='store_true')
    ...
    ddim_sampler = DDIMSampler(model.module, tgt_scale=args.tgt_scale, guide_gaze=args.guide_gaze, guide_fq=args.guide_fq)

    # Initialize guide
    dict_guide = {}
    if args.guide_gaze:
        print('INFO: Initialize Gaze Guiding!')
        netGaze = Gaze_estimator().to(device)
        dict_guide.update({'gaze':netGaze})
        fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False,device='cpu')
        dict_guide.update({'fa':fa})
    
    if  args.guide_fq:
        print('INFO: Using Frequency Domain Guiding!')


    if not args.guide_gaze and not args.guide_fq:
        dict_guide=None
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):        
        ...
        perform_swap(model.module, batch, ckpt, ddim_sampler, dict_guide)
