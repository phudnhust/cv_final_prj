...
def perform_swap(self, batch, ckpt, ddim_sampler = None, dict_condfn={}, ddim_steps=200, ddim_eta=0., **kwargs):
    ...
    for mask_key in ['mask']:
        ...
        plot_loss(ddim_sampler.list_GazeLoss)
...
def plot_loss(list_lossGaze):
    plt.plot(list_lossGaze)
    plt.savefig('loss_gaze.png')

if __name__ == '__main__':
    batch_size = 16
    num_workers = 8
    root_dir = 'data/portrait_guide'
    save_dir = root_dir
    ...
    spNorm = SpecificNorm()
    dict_guide.update({'spnorm':spNorm})
    netSeg = BiSeNet(n_classes=19).to(device)
    netSeg.load_state_dict(torch.load('./checkpoints/FaceParser.pth'))
    netSeg.eval()
    dict_guide.update({'segment':netSeg})
