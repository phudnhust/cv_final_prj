    ...
    def edit_image_by_prompt(self):
        def cond_fn(x, t, img_id, y=None):
            with torch.enable_grad():
                ...
                mask_brow = targ_seg[:,3,:,:]+targ_seg[:,2,:,:]
                mask_brow  = transforms.Resize((256,256))(mask_brow)
                mask_brow = torch.where(mask_brow > 7.0, 0.0001, 1.0)
                mask_eye = targ_seg[:,4,:,:]+targ_seg[:,5,:,:]
                mask_eye  = transforms.Resize((256,256))(mask_eye)
                mask_eye = torch.where(mask_eye > 7.0, 0.0001, 1.0)
                control_mask = (self.args.lamda_brow*mask_brow + self.args.lamda_eye*mask_eye)
                ...
                #fft
                src_mask_f  = (x_in + 1) / 2
                src_mask_f *=control_mask
                img_gray_s = torchvision.transforms.functional.rgb_to_grayscale(src_mask_f)
                #f_s = np.fft.fft2(img_gray_s)
                f_s = torch.fft.fft2(img_gray_s)
                #fshift_s = torch.fft.fftshift(f_s)

                targ_mask_f = (self.targ_image + 1) / 2
                targ_mask_f*=control_mask
                img_gray_t = torchvision.transforms.functional.rgb_to_grayscale(targ_mask_f)
                f_t =  torch.fft.fft2(img_gray_t)
                #fshift_t = torch.fft.fftshift(f_t)
                loss = (loss + complex_mse_loss(f_s, f_t)*10)
                
                # src_mask_f  = (x_in + 1) / 2
                # #print(src_mask_f.shape)
                # #f_s = np.fft.fft2(img_gray_s)
                # f_s0 = torch.fft.fft2(src_mask_f[:,0,:,:])
                # f_s1 = torch.fft.fft2(src_mask_f[:,1,:,:])
                # f_s2 = torch.fft.fft2(src_mask_f[:,2,:,:])
                # #fshift_s = torch.fft.fftshift(f_s)

                # #fshift_s = np.fft.fftshift(f_s)
                # targ_mask_f = (self.targ_image + 1) / 2
                # f_t0 =  torch.fft.fft2(targ_mask_f[:,0,:,:])
                # f_t1 =  torch.fft.fft2(targ_mask_f[:,1,:,:])
                # f_t2 =  torch.fft.fft2(targ_mask_f[:,2,:,:])

                # #fshift_t = torch.fft.fftshift(f_t)
                # #print(fshift_s)
                # loss = (loss + complex_mse_loss(f_s0, f_t0)+ complex_mse_loss(f_s1, f_t1)+ complex_mse_loss(f_s2, f_t2))
